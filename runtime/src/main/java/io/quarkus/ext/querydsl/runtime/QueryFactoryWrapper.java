...
public class QueryFactoryWrapper<Q extends AbstractSQLQuery<?, ?>, F extends AbstractSQLQueryFactory<Q>>
    ...
    public QueryFactoryWrapper(F delegate) {
    ...
    public SQLDeleteClause delete(RelationalPath<?> path) {
        if (delegate instanceof QueryFactoryWithLog) {
            return ((QueryFactoryWithLog) delegate).deleteWithLog(path);
        } else {
    ...
    public SQLInsertClause insert(RelationalPath<?> path) {
        if (delegate instanceof QueryFactoryWithLog) {
            return ((QueryFactoryWithLog) delegate).insertWithLog(path);
        } else {
    ...
    public SQLUpdateClause update(RelationalPath<?> path) {
        if (delegate instanceof QueryFactoryWithLog) {
            return ((QueryFactoryWithLog) delegate).updateWithLog(path);
        } else {
